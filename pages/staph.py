...
def normaliser_motif(motif, risque):
    m = str(motif).lower()
    r = str(risque).lower()
    if "staphylocoque" in m or "staph" in r : 
        return "Présence de staphylocoques"
    return "Autre motif"
...
st.title("Focus : :red[Présence de staphylocoques]")
st.divider()
...
with col_gauche:
    ...
    with st.container(border=True):
        ...
        st.info("""
        Staphylococcus aureus est une bactérie couramment présente sur la peau, dans le nez et la gorge des humains et des animaux.
        Elle peut contaminer les aliments via la manipulation humaine ou via des animaux infectés (mammites chez les vaches).

        👉 Le danger ne vient pas directement de la bactérie elle-même, mais des entérotoxines qu’elle produit dans les aliments.
        Ces toxines sont thermorésistantes : elles ne sont pas détruites par la cuisson.
        """)
        ...
        * 🫗 S'hydrater abondemment (risque de déshydratation)
        * 🔙 Ramener le produit au point de vente (ou le jeter)
        * 😷 Surveiller l'apparition de symptômes pendant les jours suivants (> 48h)
        * 👨‍⚕️ Consulter sans délai un médecin en cas d'apparition de symptômes ou si personne à risque
        """)
        ...
        * 🆘 **Numéro d’urgence européen :** 112
        ...
        st.markdown("""
        **Sources :**
        * World Health Organization, 2024. Food safety. Date de consultation : 16/04/2026. Disponible sur : <https://www.who.int/news-room/fact-sheets/detail/food-safety>.
        """)
...
with col_droite:
    ...
    if not data_rappels.empty:
        ...
        st.info("Aucun rappel récent trouvé pour présence de staphylocoques.")
