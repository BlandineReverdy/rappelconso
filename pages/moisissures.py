...
def normaliser_motif(motif, risque):
    m = str(motif).lower()
    r = str(risque).lower()
    if "azote" in m or "moisissures" in m or "moisi" in r : 
...
st.title("Focus : :red[Présence de moisissures]")
st.divider()
...
with col_gauche:
    ...
    with st.container(border=True):
        ...
        st.info("""
        La moisissure est un champignon microscopique qui se développe à la surface ou à l'intérieur
        de l'aliment. Attention si vous en voyez sur un aliment, couper la partie visuellement touchée 
        ne suffit pas car les racines se propagent en profondeur. Il faut donc JETER tous le produit.
        Les moisissures se développent suite à un défaut d'emballage (micro-trou, mal scellé, ...),
        une rupture de la chaîne du froid ou un problème d'humidité.
        """)
        
        st.write("""
        **Risques pour la santé :**
        Il peut y avoir des troubles digestifs (nausées, vomissements ou diarrhées), un risque allergique (réactions cutanées ou respiratoires)
        Certaines moisissures produisent des mycotoxines invisibles et dangereuses pouvant causer une intoxication.
        """)
        ...
        * 👨‍⚕️ Consulter sans délai un médecin en cas de symptômes persistants ou de risque allergique
        * 🔙 Ramener le produit au point de vente (ou jeter tout le produit)
        """)
        ...
        * 🆘 **Numéro d’urgence européen :** 112
        ...
        st.markdown("""
        **Sources :**
        * Organisation Mondiale de la Santé, 2023. Principaux repères sur les mycotoxines. Date de consultation : 16/04/2026. Disponible sur : <https://www.who.int/fr/news-room/fact-sheets/detail/mycotoxins>.
        """)
...
with col_droite:
    ...
    if not data_rappels.empty:
        ...
        st.info("Aucun rappel récent trouvé pour présence de moisissures.")
