...
def normaliser_motif(motif, risque):
    m = str(motif).lower()
    r = str(risque).lower()
    if "listeria" in m or "listéria" in m : 
...
st.title("Focus : :red[Présence de Listeria]")
st.divider()
...
with col_gauche:
    ...
    with st.container(border=True):
        ...
        st.info("""
        La listériose est une infection liée à la présence de la bactérie Listeria monocytogenes dans 
        certains aliments (produits laitiers au lait cru, charcuteries, poissons fumés). Cette bactérie 
        a la particularité de pouvoir se développer même à basse température, notamment au réfrigérateur.
        La contamination survient généralement lors d’une mauvaise hygiène ou d’une conservation prolongée.
        """)
        
        st.write("""
        **Risques pour la santé :**
        Selon la sensibilité des personnes, les effets peuvent varier. Les symptômes incluent fièvre, maux de tête 
        et troubles digestifs. Chez les personnes fragiles (femmes enceintes, personnes âgées, immunodéprimées), 
        l’infection peut entraîner des complications graves comme des atteintes neurologiques ou des infections généralisées.
        """)
        ...
        * 🧽 Nettoyer le réfrigérateur et les surfaces en contact 
        """)
        ...
        * 🆘 **Numéro d’urgence européen :** 112
        ...
        st.markdown("""
        **Sources :**
        * Institut Pasteur, 2024. Listériose. Date de consultation : 16/04/2026. Disponible sur : <https://www.pasteur.fr/fr/centre-medical/fiches-maladies/listeriose>.
        """)
...
with col_droite:
    ...
    if not data_rappels.empty:
        ...
        st.info("Aucun rappel récent trouvé pour présence de Listeria.")
