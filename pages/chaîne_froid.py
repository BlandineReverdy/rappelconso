...
def normaliser_motif(motif, risque):
    m = str(motif).lower()
    r = str(risque).lower()
    if "température" in m or "froid" in m or "rupture chaîne" in r : 
        return "Rupture de la chaîne du froid"
    return "Autre motif"
...
st.title("Focus : :red[Rupture de la chaîne du froid]")
st.divider()
...
with col_gauche:
    ...
    with st.container(border=True):
        ...
        st.info("""
        On parle de rupture de la chaîne du froid quand un produit réfrigéré ou surgelé a été exposé à 
        des températures supérieures à celles recommandées (généralement +4°C pour le frais et -18°C pour
        le surgelé) pendant une durée trop longue. Cela peut arriver durant le transport, le stockage en 
        magasin ou en cas de panne de chambre froide en usine.
        """)
        
        st.write("""
        **Risques pour la santé :**
        Le risque est qu'il y ai une prolifération bactérienne, c'est à dire une multiplication rapide de germes 
        comme la Listeria ou la Salmonelle. Il y a donc un risque de diarrhées, fièvre, maux de tête ou 
        complications graves (pour les femmes enceintes, personnes âgées).
        """)
        ...
        * 😷 Surveiller les symptômes (fièvre, troubles digestifs)
        ...
        * 🆘 **Numéro d’urgence européen :** 112
        ...
        st.markdown("""
        **Sources :**
        * Ministère de l’Agriculture, de l’Agro-alimentaire et de la Souveraineté alimentaire, 2025. Sécurité sanitaire des aliments : tout sur la chaîne du froid. Date de consultation : 16/04/2026. Disponible sur : <https://agriculture.gouv.fr/securite-sanitaire-des-aliments-tout-sur-la-chaine-du-froid>.
        """)
...
with col_droite:
    ...
    if not data_rappels.empty:
        ...
        st.info("Aucun rappel récent trouvé pour rupture de la chaîne du froid.")
