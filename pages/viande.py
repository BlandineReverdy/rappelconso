...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="Focus viandes", page_icon="🥩", layout="wide")
...
# --- 2. CSS PERSONNALISÉ (Inclus le style pour les liens) ---
st.markdown("""
<style>
    ...
    .main-card {
        ...
        height: 360px; /* Fixe la hauteur pour l'alignement */
    }
    .img-container {
        height: 150px;
        display: flex;
...
# --- 3. MAPPING DES PAGES (Pour les liens cliquables) ---
pages_motifs = {
...
def normaliser_motif(motif, risque):
    m = str(motif).lower()
    r = str(risque).lower()
    if "listeria" in m or "listéria" in r: return "Présence de Listeria"
...
if not data_fish.empty:
    ...
    with col_graph:
        ...
        with st.expander("🔎 Outils de Filtrage - Affinez votre analyse", expanded=True):
            ...
            with f_col1:
                recherche_texte = st.text_input("Recherche libre (ex: Poulet, Carrefour...)", "")
        ...
        # Ton bloc de calcul de métriques original
        ...
        if total_rappels > 0:
            ...
            with tab1:
                # Ton calcul Sunburst complexe avec "Autres risques"
            ...
            st.markdown(f"""
                <div class="main-card">
                    <div style="display: flex; justify-content: space-between; color: #6b7280; font-size: 0.75rem; font-weight: bold;">
                        ...
                        <span style="color: #f59e0b;">📂 VIANDE</span>
                    </div>
