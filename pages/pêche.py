...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="Dashboard Pêche & Aquaculture", page_icon="🐟", layout="wide")
...
# --- 2. CSS PERSONNALISÉ (Alignement et liens cliquables) ---
st.markdown("""
<style>
    ...
    .main-card {
        ...
        height: 350px; /* Fixe la hauteur pour l'alignement des cartes */
    }
    .img-container {
        height: 150px;
        display: flex;
...
# --- 3. MAPPING DES PAGES POUR LES LIENS ---
pages_motifs = {
...
with col_title_header:
    st.title("🐟 Focus : :red[Pêche et Aquaculture]")
...
if not data_fish.empty:
    ...
    with col_graph:
        ...
        with st.expander("🔎 Outils de Filtrage - Affinez votre analyse", expanded=True):
            ...
            with f_col1:
                recherche_texte = st.text_input("Recherche libre (ex: Saumon, Crevettes, Carrefour...)", "")
    ...
    with col_last_news:
        ...
        st.warning("⚠️ Rappel important : Si vous possédez l'un de ces produits, ne le consommez pas.")
            ...
            st.markdown(f"""
                <div class="main-card">
                    <div style="display: flex; justify-content: space-between; color: #6b7280; font-size: 0.75rem; font-weight: bold;">
                        ...
                        <span style="color: #f59e0b;">📂 PÊCHE</span>
                    </div>
