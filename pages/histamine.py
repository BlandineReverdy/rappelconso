...
# --- 1. CONFIGURATION ---
st.set_page_config(page_title="Fiche Rappel - Présence d'histamine", layout="wide")
...
def normaliser_motif(motif, risque):
    m = str(motif).lower()
    r = str(risque).lower()
    if "histamine" in m or "histamine" in r : 
        return "Présence d'histamine"
    return "Autre motif"
...
st.title("Focus : :red[Présence d'histamine]")
st.divider()
...
with col_gauche:
    ...
    with st.container(border=True):
        ...
        st.info("""
        Le poisson contient naturellement un acide aminé inoffensif appelé l'histidine.
        Dès que le poisson meurt, si la chaîne du froid est brisée (température > 4°C), des bactéries
        commencent à se multiplier, et grignote l'histidine pour la transformer en histamine.
        L'histamine est invisible, inodore et ne change pas le goût du poisson. Elle résiste à la 
        cuisson, à la congélation et à la mise en conserve.
        """)
        
        st.write("""
        **Risques pour la santé :**
        En 10 à 30 minutes, le visage et le cou deviennent tout rouges et brûlants. Maux de tête, 
        fourmillements sur la langue, accélération du cœur et souvent une diarrhée immédiate (le 
        corps veut expulser le "poison").
        """)
        
        st.markdown("""
        **Conduite à tenir :**
        * ✅ Ne plus consommer le produit       
        * 🫗 S'hydrater abondemment
        ...
        * 🆘 **Numéro d’urgence européen :** 112
        ...
        st.markdown("""
        **Sources :**
        * Commission Européenne, 2006. Règlement (CE) n° 2073/2005. Date de consultation : 16/04/2026. Disponible sur : <https://www.economie.gouv.fr/files/files/directions_services/dgccrf/securite/produits_alimentaires/Hygiene_alimentaire/ha-reg-2073.pdf>.
        """)
...
with col_droite:
    ...
    if not data_rappels.empty:
        ...
        st.info("Aucun rappel récent trouvé pour la présence d'histamine.")
